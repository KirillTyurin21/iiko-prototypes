import { ChangelogRelease } from '@/shared/changelog.types';

export const CHANGELOG: ChangelogRelease[] = [
  {
    version: '1.5',
    date: '2026-04-15',
    status: 'released',
    changes: [
      {
        page: 'Подсказки — Сравнение вариантов',
        pageRoute: '/prototype/front-plugins/hints/variants',
        items: [
          'Переработана информационная структура суб-вариантов «Два квадрата» по замечаниям Руслана',
          'Метка сверху: «Подсказка для блюда «{триггер-блюдо}»» — кассир сразу видит контекст',
          'Инструкция кассиру: «Предложите клиенту:» вместо абстрактного слогана',
          'Убран заголовок «Специальное предложение» и текст «Вы добавили...» из контента',
          'Кнопки пропорциональные (90px) вместо растянутых квадратов',
        ],
      },
    ],
  },
  {
    version: '1.4',
    date: '2026-04-14',
    status: 'released',
    changes: [
      {
        page: 'Подсказки — Сравнение вариантов',
        pageRoute: '/prototype/front-plugins/hints/variants',
        items: [
          'Добавлен блок «Вариации Два квадрата» с 5 суб-вариантами выбранного дизайна',
          'Суб-варианты: увеличенная картинка (150px), крупная (180px), hero-баннер, акцентная кнопка, компактный баланс (140px)',
          'Реальные фото продуктов вместо emoji-заглушек',
          'Кнопки «Отказаться» и «Добавить» — только текст белого цвета, без иконок',
        ],
      },
    ],
  },
  {
    version: '1.3',
    date: '2026-04-09',
    status: 'released',
    changes: [
      {
        page: 'Подсказки — Сравнение вариантов',
        pageRoute: '/prototype/front-plugins/hints/variants',
        items: [
          'Добавлен вариант 3: «Иконка-кнопка» — квадратная кнопка с иконкой корзины справа от контента, «Отказаться» на всю ширину',
          'Добавлен вариант 4: «Два квадрата» — два больших квадрата внизу: «Отказаться» слева, «Добавить» справа с иконками',
          'Таблица сравнения расширена до 5 вариантов',
        ],
      },
    ],
  },
  {
    version: '1.2',
    date: '2026-04-08',
    status: 'released',
    changes: [
      {
        page: 'Подсказки — Демо',
        pageRoute: '/prototype/front-plugins/hints/demo',
        items: [
          'Каждое блюдо в меню теперь показывает свою уникальную подсказку с индивидуальной рекомендацией',
          'При нажатии на блюдо оно сразу добавляется в чек заказа',
          'Дизайн окна подсказки адаптирован под стилистику диалоговых окон Front: прямые углы, жёлтые заголовки, чёрные кнопки',
          'Убраны атрибуты блюда (вес, ккал), убран дублирующий текст рекомендации',
        ],
      },
      {
        page: 'Подсказки — Сравнение вариантов',
        pageRoute: '/prototype/front-plugins/hints/variants',
        items: [
          'Заменены варианты дизайна: вместо «Акцентного баннера» — два варианта расположения кнопок основного окна',
          'Вариант 1: кнопки горизонтально (Добавить слева, Отказаться справа)',
          'Вариант 2: кнопки вертикально (Добавить сверху, Отказаться снизу)',
          '6 наборов демо-данных: со скидкой/без, с картинкой/без, длинные тексты, большая скидка',
          'Компактный POS адаптирован под стилистику карточных вариантов: прямые углы, золотой акцент, кнопки в ряд',
          'Добавлена «Анатомия карточки» — интерактивные аннотации источников данных для каждого варианта',
          'Кнопка «Показать/Скрыть источники» управляет видимостью цветных маркеров на карточке',
          'Легенда типов источников: Панель админа, Номенклатура, Автоматический, Расчёт, Составной',
        ],
      },
    ],
  },
  {
    version: '1.1',
    date: '2026-04-08',
    status: 'released',
    changes: [
      {
        page: 'Подсказки — Демо',
        pageRoute: '/prototype/front-plugins/hints/demo',
        items: [
          'Добавлен экран-имитация заказа Front с модальным окном подсказки',
          'При нажатии на блюдо в меню появляется окно допродажи с кнопками «Добавить» и «Отказаться»',
        ],
      },
      {
        page: 'Подсказки — Сравнение вариантов',
        pageRoute: '/prototype/front-plugins/hints/variants',
        items: [
          'Добавлен экран сравнения 3 дизайнов окна: «Карточка», «Баннер», «POS»',
          'Переключение между 3 наборами демо-данных (со скидкой, без картинки, комбо)',
          'Таблица сравнения критериев',
        ],
      },
      {
        items: [
          'Добавлена карточка «Digital Signage — Подсказки» на главный экран',
        ],
      },
    ],
  },
  {
    version: '1.0',
    date: '2026-02-16',
    status: 'released',
    changes: [
      {
        items: [
          'Начальная версия прототипа',
          'Макеты модальных окон плагинов Front',
        ],
      },
    ],
  },
];
