...
@Component({
  ...
  template: `
    <neptune-pos-dialog [open]="open" maxWidth="md" (dialogClose)="dialogClose.emit()">
      ...
      <!-- Title -->
      <h2 class="text-2xl font-semibold text-red-400 text-center mb-2">
        {{ scenario?.title || 'Ошибка' }}
      </h2>

      <!-- HTTP code badge -->
      <div *ngIf="scenario?.httpCode" class="flex justify-center mb-3">
        <span class="text-xs font-mono px-2 py-0.5 rounded bg-red-900/40 text-red-300">
      ...
      <!-- Action hint -->
      <div *ngIf="scenario?.action" class="bg-[#2d2d2d] rounded p-3 mb-6">
      ...
      <!-- Footer -->
      <div class="grid gap-3" [ngClass]="scenario?.retryable ? 'grid-cols-2' : 'grid-cols-1'">
        <button
