...
@Component({
  ...
  template: `
    <div class="org-tree">
      <div *ngFor="let org of organizations" class="mb-2">
        <!-- Organization node -->
        <div
          class="flex items-center gap-2 px-3 py-2 rounded cursor-pointer hover:bg-gray-100 transition-colors"
        ...
        >
          <lucide-icon
            ...
            class="text-gray-400"
          ></lucide-icon>
          <lucide-icon name="building-2" [size]="16" class="text-gray-500"></lucide-icon>
          <span class="text-sm font-medium text-gray-700">{{ org.name }}</span>
        </div>

        <!-- Stores -->
        <div *ngIf="expandedOrgs[org.id]" class="ml-4">
          <div *ngFor="let store of org.stores" class="mb-1">
            <!-- Store node -->
            <div
              class="flex items-center gap-2 px-3 py-2 rounded cursor-pointer transition-colors"
              [class.bg-blue-50]="selectedId === store.id"
              [class.border-l-2]="selectedId === store.id"
              [class.border-app-primary]="selectedId === store.id"
              [class.hover:bg-gray-50]="selectedId !== store.id"
              (click)="selectStore(store)"
            >
              <div
                class="w-2.5 h-2.5 rounded-full flex-shrink-0"
                [class.bg-green-500]="store.credentialsStatus === 'configured'"
                [class.bg-gray-300]="store.credentialsStatus === 'not-configured'"
                [class.bg-red-500]="store.credentialsStatus === 'error'"
              ></div>
              <lucide-icon name="store" [size]="15" class="text-gray-500"></lucide-icon>
              <span class="text-sm" [class.font-semibold]="selectedId === store.id">
                {{ store.name }}
              </span>
            </div>

            <!-- Terminals -->
            <div *ngIf="selectedId === store.id" class="ml-6">
              <div
                *ngFor="let term of store.terminals"
                class="flex items-center gap-2 px-3 py-1.5 text-xs text-gray-500 rounded hover:bg-gray-50 cursor-pointer"
