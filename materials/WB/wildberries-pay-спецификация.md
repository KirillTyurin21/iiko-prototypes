...
| 5 | Возвраты через ЛК WB | WB планирует реализовать возвраты через личный кабинет клиента (web-интерфейс) во 2-м квартале 2026 (по данным WB). Потенциально снижает нагрузку на кассиров - возврат инициируется гостем самостоятельно | 2Q 2026 |

#### Вне scope

| # | Функция / доработка | Причина |
|---|---------------------|---------|
| 1 | Лояльность "ягодки" (SDK-интеграция) | Логика начисления и списания ягодок полностью на стороне WB. При подтверждении оплаты в приложении WB гость видит опцию "Начислить" или "Списать" ягодки (по данным WB, встреча #2). В iiko приходит факт оплаты на 100% суммы - плагин не видит деталей баллов. Взаиморасчёты по ягодкам между WB и рестораном происходят напрямую: WB компенсирует ресторану полную сумму за вычетом процента утилизации баллов, без участия iiko (по данным WB). Дополнительная логика в плагине не требуется |
| 2 | Оффлайн-режим (без интернета) | Плагин WB Pay работает через REST API, каждая оплата требует обращения к серверу WB Pay в реальном времени. Без интернета регистрация и подтверждение оплаты невозможны. Отклонено iiko на встрече 18.02.2026 |
| 3 | География за пределами РФ | Плагин работает только на территории Российской Федерации. Валюта - только RUB (код 643) |
| 4 | Токенизация (повторные оплаты без QR) | Offline API WB Pay не содержит эндпоинтов для создания и использования платежного токена. Функция доступна только для online-сценариев (интернет-платежи). Целевой кейс токенизации - фастфуд (регулярные покупки, лимит 6 000 рублей за транзакцию). Подтверждено WB |
...
| TTL JWT-токена | 1 год. WB уведомляет клиентов за 1 месяц до плановой замены через почту/каналы поддержки (по данным WB). Обновление - ручное, через поддержку WB |
| terminal_id | 1 terminal = 1 точка приёма оплат. Гибкая привязка: допускается 1 terminal_id на несколько касс или по 1 terminal_id на каждую кассу - зависит от потребностей клиента (по данным WB). Параллельные оплаты с одного terminal_id возможны |
...
- **JWT (bearer-токен)** - строка авторизации для всех запросов к API; TTL = 1 год (по данным WB)
...
| 12 | При превышении таймаута polling (конфигурируемый параметр PollingTimeoutSec) плагин должен завершить `Pay()` с ошибкой. WB гарантирует финализацию операции до 24 часов (по данным WB) |
| 13 | При HTTP 403 на любом запросе плагин должен показать кассиру сообщение об ошибке авторизации |
| 14 | Плагин должен передавать заголовки X-Request-Country и X-Request-Region в запросе регистрации оплаты [ДОПУЩЕНИЕ] |
...
- Максимальное время ожидания: конфигурируемый (PollingTimeoutSec, по умолчанию 120 секунд). WB гарантирует финализацию операции до 24 часов (по данным WB)
...
| Сетевая ошибка (timeout, connection refused) | Retry по стратегии (см. API-справочник, раздел 2.4). Метод /do идемпотентен - безопасный retry при сетевом сбое (подтверждено WB). При исчерпании попыток - PaymentActionFailedException |
...
> [!WARNING]
> Заголовок X-Wbpay-Id при проверке статуса возврата содержит **order_id** (не refund_id). Это подтверждено документацией WB Pay.
...
| Сетевая ошибка (timeout, connection refused) | Retry по стратегии (см. API-справочник, раздел 2.4). Методы /do идемпотентны - безопасный retry при сетевом сбое (подтверждено WB). PaymentActionFailedException |
...
Плагин Wildberries Pay использует стандартные диалоговые окна iiko SDK (`IViewManager`). Кастомный UI (собственные экраны, формы, Avalonia) не предусмотрен - решение принято на встрече #1 (28.01.2026). Все визуальные элементы отображаются средствами iikoFront.
...
iikoWeb REST API (iikoWeb Frontend ->> iikoWeb Backend) описан по сценарию В: API спроектировано для страницы WB Pay в iikoWeb (раздел 6.5). 9 методов организованы в 3 контроллера (Credentials, Store, Terminal) + 1 internal endpoint (Plugin pull). Проектирование выполнено на основе паттернов YandexPay из Swagger-документации iikoWeb.
...
> [!WARNING]
> Заголовок `X-Wbpay-Id` при проверке статуса возврата содержит **order_id** (не refund_id). Это подтверждено документацией WB Pay и описано в API-справочнике (раздел 3.6).

#### Закрытые вопросы

| # | Уточнение | Решение |
|---|-----------|---------|
| 1 | Источник значения `invoice_id` для методов register (оплата и возврат) | `IOrder.Id` (GUID заказа iiko). по данным WB |
...
| TerminalId | string | Да | - | Идентификатор точки приёма оплат, выданный WB Pay. Гибкая привязка: 1 terminal_id на несколько касс или по 1 на каждую (по данным WB) | iikoWeb (Transport) / QR-проливка / Ручной ввод | 7.1, п.2; 7.2, п.3 |
...
| Параметр | Тип | Обяз. | По умолч. | Описание | Связь |
|----------|-----|:-----:|-----------|----------|:-----:|
| HttpTimeoutSec | int | Нет | 10 | Таймаут одного HTTP-запроса в секундах [ДОПУЩЕНИЕ] | API-справочник, раздел 2.4 |
...
| PollingTimeoutSec | int | Нет | 120 | Общий таймаут ожидания финального статуса polling (в секундах). WB гарантирует финализацию до 24 часов (по данным WB). Значение по умолчанию 120 сек обусловлено UX-ограничениями кассы. Ресторан может увеличить при необходимости | 7.1, п.12; 7.2, п.5 |
...
| # | Уточнение | Решение |
|---|-----------|---------|
| 1 | Стратегия retry и идемпотентность | /do идемпотентен, безопасный retry (по данным WB). Параметры остаются как есть |
| 2 | Рекомендуемый таймаут polling от WB | WB гарантирует финализацию до 24ч (по данным WB). Плагин: 120 сек по умолчанию, конфигурируемо |
...
|----------|-----------------------|----------|
| RetryCount | 3 | Максимальное количество повторных попыток [ДОПУЩЕНИЕ] |
| RetryBaseIntervalMs | 1000 | Базовый интервал ожидания (мс) [ДОПУЩЕНИЕ] |
...
Метод `/do` (подтверждение оплаты и возврата) идемпотентен - повторный вызов с тем же order_id / refund_id безопасен и не приводит к двойному списанию (по данным WB).
...
WB Pay гарантирует, что заказ может находиться в статусе pending до 24 часов на стороне сервера (по данным WB). Плагин ограничен настройкой PollingTimeoutSec (по умолчанию 120 сек). Если за это время финальный статус не получен, плагин прекращает polling и отклоняет операцию. Статус заказа на стороне WB при этом может измениться позднее - данная ситуация остается вне контроля плагина.
...
Таймаут одного HTTP-запроса: 10 секунд (параметр HttpTimeoutSec в config.xml) [ДОПУЩЕНИЕ].
...
- Токенизация (повторные оплаты без QR) недоступна в Offline API WB Pay. Каждая оплата требует сканирования нового QR-кода (по данным WB)
...
- Параметры retry (RetryCount = 3, exponential backoff 1/2/4 сек) и таймаут HTTP-запроса (HttpTimeoutSec = 10 сек) являются допущениями аналитика. Оптимальные значения определяются на этапе реализации [ДОПУЩЕНИЕ]
- Заголовки X-Request-Country и X-Request-Region передаются в каждом запросе к WB Pay API. Значения задаются в конфигурации плагина [ДОПУЩЕНИЕ]
