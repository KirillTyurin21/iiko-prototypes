...
| Связанные документы | |
|------|---|
| API-справочник | [MGS-API-справочник.md](MGS-API-справочник.md) |
| Открытые вопросы | [Открытые-вопросы.md](../03_Рабочие_материалы/Вопросы/Открытые-вопросы.md) |
| Метаданные | [MGS-метаданные.md](MGS-метаданные.md) |
...
Настоящая спецификация описывает требования к адаптации плагина NeonPlugin для кассового терминала iikoFront (API V9 LTS). Плагин адаптируется для работы с новой системой управления казино MGS вместо устаревшей системы Neon. Документ предназначен для команды разработки iiko, QA-инженеров, заказчика и технической поддержки.

| Параметр | Значение |
|----------|----------|
| Заказчик | DaGame (Максим Плешков) - оператор казино |
| Техническая сторона | MGS (Максим Зобин - API, Екатерина - координация) |
| Версия API | V9 (LTS) |
| Тип решения | Адаптация существующего плагина iikoFront |
| Целевой срок | Март 2026 (первый объект) |
...
- Оплату заказа баллами лояльности, кэш-поинтами и комплиментарными баллами
- Применение статусных скидок
- Запись данных о госте в External Data заказа

Заказчик переходит с системы управления казино Neon на новую систему MGS. Переход поэтапный, первый объект - март 2026. Плагин NeonPlugin ДОЛЖЕН быть адаптирован для работы с REST/JSON API системы MGS вместо SOAP/WCF-сервисов Neon.
...
- Начисление средств на баланс гостя (CREDIT) - запрещено через плагин; начисление выполняет только РКЦ казино (источник: встреча 11.03.2026)
- Скидки по статусу гостя - отложены за пределы MVP; в текущей версии MGS скидки отсутствуют (источник: встреча 11.03.2026)
- Возврат средств - вне MVP; возврат реализуется через систему заявок в кассу казино, не через API MGS. Endpoint возврата будет добавлен в MGS позднее (источник: встреча 11.03.2026)
...
| Кэш-поинты (cash) | Денежный эквивалент на счёте гостя казино, используемый для безналичной оплаты в ресторане. В API MGS - тип WalletPayment (endpoint /v1/payment/cash) |
| Баллы лояльности (promo/points) | Бонусные баллы нескольких типов, начисляемые гостю за активность. Каждый тип имеет point_id и привязан к point_service. В API MGS - тип PointPayment (endpoint /v1/payment/promo) |
| Типы баллов (point_id) | В MGS баллы лояльности разделены на 4 типа: Complimentary (0), Gaming (1), Travel (3), Restaurant (4). Плагин работает только с Restaurant и Complimentary. Конкретные числовые значения point_id параметризуются в конфигурации (на разных объектах казино могут отличаться) |
| Комплиментарные баллы (comp) | Специальный тип баллов лояльности (point_id=0, Complimentary). Начисляются менеджером казино вручную через интерфейс MGS для покрытия счёта гостя. Плагин отображает баланс и позволяет списывать |
...
> Протокол авторизации и способы аутентификации подробно описаны в [MGS-API-справочник.md](MGS-API-справочник.md), раздел 1.

> [!NOTE]
> Доступ к тестовой среде осуществляется через L2TP VPN. Данные для подключения не предоставлены; текущая спецификация составлена на основе openapi.yaml.
...
Полное описание каждого метода: [MGS-API-справочник.md](MGS-API-справочник.md)
...
| Типы баллов | 3 отдельных баланса (Cashless, Loyalty, Comp) | Общий баланс (cash) + N типов баллов (points[]) + каталог сервисов | Более гибкая модель, сложнее маппинг |
...
Перед любой платёжной операцией гость ДОЛЖЕН ввести PIN-код. Плагин отправляет PIN-код в MGS (POST /v1/payment/get_token) и получает токен со сроком действия 5 минут. Токен передаётся в каждом платёжном запросе (/v1/payment/cash, /v1/payment/promo, /v1/payment/balance).

*На основе предоставленного API MGS (endpoint /v1/payment/get_token).*

**Множественные типы баллов**

В MGS баллы лояльности разделены по типам (point_id): Complimentary (0), Gaming (1), Travel (3), Restaurant (4). Плагин работает только с двумя типами: Restaurant (оплата в ресторане) и Complimentary (подарочные списания, начисляются менеджером казино вручную). Конкретные числовые значения point_id параметризуются в конфигурации плагина (на разных объектах казино могут отличаться).
...
*На основе предоставленного API MGS (схемы PointPayment, BalanceResponse) и решений встречи 11.03.2026.*
...
| 5 | - | Шаг отложен за пределы MVP. Скидки по статусу гостя не реализуются в текущей версии (решение встречи 11.03.2026) | - |

> [!NOTE]
> Все платёжные операции плагина поддерживают только списание (DEBIT). Начисление (CREDIT) запрещено - эта функция доступна исключительно через РКЦ казино (решение встречи 11.03.2026).
...
> [!WARNING]
> Скидки по статусу гостя отложены за пределы MVP (решение встречи 11.03.2026, Максим Зобин, MGS: "сейчас у нас нету скидок"). В текущей версии API MGS механизм скидок отсутствует. MGS планирует добавить скидки после MVP. Шаг пропускается при выполнении сценария.
...
|-------------------|-----|----------|
| point_id | integer | ID типа баллов. Плагин работает с двумя типами: Restaurant (параметр конфига `RestaurantPointId`) и Complimentary (параметр конфига `ComplimentaryPointId`). Значения параметризованы, не захардкожены (решение встречи 11.03.2026) |
...
| Поле запроса | Тип | Источник |
|-------------|-----|----------|
| point_id | integer, int32 | Параметр конфига `RestaurantPointId` (решение встречи 11.03.2026: point_id параметризуется в конфиге, НЕ хардкодится) |
...
#### Сценарий 7.1.4: Оплата комплиментарными баллами

> [!NOTE]
> Комплиментарные баллы начисляет менеджер казино вручную через собственный интерфейс MGS, НЕ через плагин iiko. Плагин только отображает баланс комплиментарных баллов и позволяет их списывать (только DEBIT). Это "неживые деньги" (баллы лояльности), начисляемые менеджером по согласованию (решение встречи 11.03.2026, Максим Зобин, MGS).

| Шаг | Действие пользователя | Реакция системы | Результат |
|:---:|----------------------|-----------------|-----------|
| 1 | Кассир нажимает кнопку "Comp" (оплата комплиментарными баллами) | Плагин проверяет наличие действующего токена | Токен проверен/получен |
| 2 | - | Плагин запрашивает баланс и проверяет доступность комплиментарных баллов (point_id из параметра конфига `ComplimentaryPointId`) | Баланс получен |
| 3 | Кассир подтверждает сумму списания (только DEBIT - списание) | Плагин отправляет POST /v1/payment/promo (point_id, point_service_id, amount, token) | Запрос списания отправлен |
| 4 | - | MGS выполняет списание и возвращает подтверждение | Списание выполнено |
| 5 | - | Плагин фиксирует оплату в заказе iikoFront и добавляет строку списания на чек | Оплата зафиксирована, чек обновлён |

**Предусловия**
- Гость идентифицирован (сценарий 7.1.1 выполнен)
- У кассира есть роль, разрешающая оплату комплиментарными баллами (use_comp_role)
- PIN-авторизация пройдена (токен действителен)
- У гостя имеются комплиментарные баллы (начислены менеджером казино)

**Постусловия**
- Комплиментарные баллы списаны
- Оплата зафиксирована в заказе iikoFront
...
| Ситуация | Реакция системы |
|----------|------------------|
| Недостаточно комплиментарных баллов (MGS возвращает ошибку) | Плагин отображает сообщение "Недостаточно комплиментарных баллов". Кассир МОЖЕТ выбрать другой тип оплаты |
| Токен истёк (TTL 5 мин превышен) | Плагин запрашивает повторный ввод PIN-кода |
| Некорректный point_id (MGS возвращает ошибку) | Плагин отображает сообщение об ошибке от MGS |

> [!NOTE]
> Значение point_id для комплиментарных баллов берётся из параметра конфигурации `ComplimentaryPointId`. Конкретное числовое значение задаётся при развёртывании на объекте (решение встречи 11.03.2026: point_id параметризуется в конфиге, НЕ хардкодится, т.к. на разных объектах казино значения могут отличаться).

**Детальная логика обработки шагов**

**Шаг 1. Проверка токена**

Кассир нажимает кнопку "Comp". Плагин проверяет роль и наличие действующего токена.

> **Проверка роли:** `use_comp_role` (конфиг, ФТ 8.10, п.5). Пустое значение = доступ всем.

- **Кнопка:** текст - `comp_button` (конфиг, по умолчанию "Списать комплиментарные баллы")
- **Оформление:** фон - `comp_button_background` (#ffffff), текст - `comp_button_foreground` (#000000)

**Логика проверки токена** (ФТ 8.3, п.5):
- Если действующий токен существует (менее 5 мин) ->> используется существующий
- Если токен отсутствует или истёк ->> открывается диалог PIN-кода (секция 9.3):
  - **API-вызов:** `POST /v1/payment/get_token` (MGS REST API)
  - Поля запроса: `customer` (из `Customer.customer_id`), `pin` (ввод гостя)

**Шаг 2. Запрос баланса и проверка комплиментарных баллов**

Плагин запрашивает баланс и извлекает данные по комплиментарным баллам.
...
- **Ответ:** HTTP 201, объект `BalanceResponse`
- **Целевой элемент:** из `points[].points` извлекается запись с `point_id` равным значению параметра конфига `ComplimentaryPointId` (решение встречи 11.03.2026: point_id параметризуется)
- **Баланс:** значение `point_sum` для этого элемента

> **Роль видимости:** `show_comp_role` (конфиг, ФТ 8.10, п.10).

**Шаг 3. Формирование и отправка запроса списания**

Кассир подтверждает сумму. Плагин отправляет запрос списания комплиментарных баллов.
...
| Поле запроса | Тип | Источник |
|-------------|-----|----------|
| point_id | integer, int32 | Параметр конфига `ComplimentaryPointId` (решение встречи 11.03.2026: не хардкодить) |
| point_service_id | integer, int32 | Конфиг `point_service_id` (раздел 12.2, каталог `GET /v1/payment/point_services`) [ТРЕБУЕТ УТОЧНЕНИЯ - З-7] |
| description | string | Формируется плагином [ДОПУЩЕНИЕ - формат текста и источник номера заказа (`IOrder.Number` или `IOrder.Id`) не документированы] |
| amount | integer, int32 | Расчет на основе `IOrder.ResultSum` (iiko SDK). Преобразование: decimal ->> integer [ТРЕБУЕТ УТОЧНЕНИЯ - И-4] |
| token | string | Токен из `POST /v1/payment/get_token` |

**Шаг 4. Обработка подтверждения списания**

MGS выполняет списание и подтверждает операцию.

- **Ответ:** HTTP 201 (формат тела не документирован в API MGS)

**Шаг 5. Фиксация оплаты в заказе iikoFront и печать на чеке**

Плагин фиксирует результат операции в iikoFront и печатает данные на чеке.

- **Фиксация оплаты:** в заказе iikoFront (ФТ 8.6, п.3)
- **Фискальный чек:** строка списания комплиментарных баллов (ФТ 8.9, п.1)
- **Пречек** (если настроен): шаблон `precheque_text` (конфиг, ФТ 8.9, п.2)
  - Переменные подстановки: `{result_sum}`
  - Переменные `{discount_percent}`, `{discount_sum}` недоступны в MVP (скидки отложены)
...
> [!NOTE]
> Поле `image` (URL фото) в этом методе НЕ возвращается (в отличие от /v1/search/card и /v1/search/id).
...
- **Фото:** недоступно (поле `image` не возвращается в этом методе)
- **Далее выполняется логика сценария 7.1.1 начиная с шага 3:**
  ...
  - Шаг 5 (скидки) отложен за пределы MVP (решение встречи 11.03.2026)
...
#### Сценарий 7.2.4: Оплата комплиментарными баллами

> [!NOTE]
> Комплиментарные баллы начисляет менеджер казино вручную через собственный интерфейс MGS, НЕ через плагин iiko. Плагин только отображает баланс и списывает (только DEBIT). См. сценарий 7.1.4 для подробностей.
...
| 3 | Кассир подтверждает сумму списания (только DEBIT - списание) | Плагин отправляет POST /v1/payment/promo (point_id, point_service_id, amount, token) | Запрос списания отправлен |
| 4 | - | MGS выполняет списание и возвращает подтверждение | Списание выполнено |
| 5 | - | Плагин фиксирует оплату в заказе iikoFront и добавляет строку списания на чек | Оплата зафиксирована, чек обновлен |
...
|----------|------------------|
| Недостаточно комплиментарных баллов (MGS возвращает ошибку) | Плагин отображает сообщение "Недостаточно комплиментарных баллов". Кассир МОЖЕТ выбрать другой тип оплаты |
| Токен истек (TTL 5 мин превышен) | Плагин запрашивает повторный ввод PIN-кода |
| Некорректный point_id (MGS возвращает ошибку) | Плагин отображает сообщение об ошибке от MGS |

> [!NOTE]
> Значение point_id для комплиментарных баллов берётся из параметра конфигурации `ComplimentaryPointId` (решение встречи 11.03.2026: point_id параметризуется, не хардкодится).

**Детальная логика обработки шагов**

Логика идентична сценарию 7.1.4 со следующим отличием контекста: в фаст-фуде повторное использование токена типично - если кэш-поинты и/или баллы лояльности уже списаны (сценарии 7.2.2, 7.2.3), токен действует для списания комплиментарных баллов в рамках того же заказа.

Описание шагов - см. сценарий 7.1.4 ("Детальная логика обработки шагов", шаги 1-5). Все API-вызовы, параметры, источники данных и обработка ролей идентичны. Ссылки на идентификацию гостя - сценарий 7.2.1 (вместо 7.1.1).
...
| 5 | Плагин ДОЛЖЕН отображать фото гостя, загружая изображение по URL из поля image. Назначение: верификация личности гостя. Поле image отсутствует в ответе guests_in_casino | Should | API MGS |

### 8.3. PIN-авторизация гостя

| ID | Требование | Приоритет | Источник |
|----|-----------|-----------|----------|
| 1 | Плагин ДОЛЖЕН запрашивать у гостя PIN-код перед любой платёжной операцией | Must | API MGS |
...
| 3 | Плагин ДОЛЖЕН отображать баллы лояльности по типам из массива points[] (point_id, point_name, point_sum) | Must | API MGS |
| 4 | Плагин ДОЛЖЕН отображать комплиментарные баллы как отдельный тип, используя point_id из параметра конфигурации ComplimentaryPointId | Must | API MGS, действующий плагин Neon |
...
| 4 | Плагин ДОЛЖЕН поддерживать списание комплиментарных баллов, используя point_id из параметра конфигурации ComplimentaryPointId. Комплиментарные баллы начисляет менеджер казино вручную; кассир видит их в балансе и выполняет списание | Must | Действующий плагин Neon |

### 8.7. Скидки по статусу гостя

> [!WARNING]
> Скидки отложены за пределы MVP. Решение встречи 11.03.2026: "У нас сейчас нету скидок" (Максим Зобин, MGS). Функциональность будет реализована в последующих версиях при появлении механизма скидок на стороне MGS.
...
| 5 | Плагин ДОЛЖЕН разграничивать доступ к функции оплаты комплиментарными баллами (use_comp_role) | Must | Действующий плагин Neon |
...
| 10 | Плагин ДОЛЖЕН разграничивать видимость финансовых данных гостя (show_cashless_role, show_loyalty_role, show_comp_role) | Should | Действующий плагин Neon |
...
| 3 | Плагин ДОЛЖЕН поддерживать настройку текстов кнопок (cashless_button, loyalty_button, comp_button) | Could | Действующий плагин Neon |
...
| ID | Требование | Приоритет | Источник |
|----|-----------|-----------|----------|
| 1 | Возврат кэш-поинтов и баллов лояльности через плагин НЕ реализуется. В MGS возврат выполняется через систему заявок в кассу казино, а не через API (решение встречи 11.03.2026, Максим Зобин: "Это подтягивается из нашей интернальной системы") | Won't | Действующий плагин Neon |
...
| Кнопка "Comp" | Кнопка | Запуск оплаты комплиментарными баллами. Текст, фон и цвет шрифта настраиваются в конфигурации | Роль use_comp_role, гость идентифицирован |
...
| Состояние | Описание |
|-----------|----------|
| Гость не идентифицирован | Кнопки оплаты (Cashless, Loyalty, Comp) неактивны. Доступна только кнопка идентификации и список гостей |
...
| Фото гостя | Изображение | Загружается по URL из поля image. Назначение: верификация личности гостя (сотрудник проверяет, что карта принадлежит гостю). Отсутствует в ответе guests_in_casino | show_photo_role |
...
| Баллы лояльности | Список | Из массива points[] (point_name + point_sum) | show_loyalty_role |
| Комплиментарные баллы | Число | Из points[] где point_id соответствует параметру конфигурации ComplimentaryPointId. Комплиментарные баллы начисляет менеджер казино вручную; кассир видит баланс и может их списать | show_comp_role |
...
| Параметр | Значение |
|----------|----------|
| Когда отображается | Перед любой платёжной операцией (кэш-поинты, баллы лояльности, комплиментарные баллы), если действующий токен отсутствует или истёк |
| Как вызывается | Нажатие кнопки оплаты (Cashless, Loyalty, Comp) |

#### Элементы экрана

| Элемент | Тип | Описание | Валидация |
|---------|-----|----------|-----------|
| Поле ввода PIN-кода | Текстовое поле (маскированное) | Гость вводит свой PIN-код. Символы отображаются как маскированные | Непустое значение |
...
| Элемент | Тип | Описание | Валидация |
|---------|-----|----------|-----------|
| Список гостей | Таблица/список | ФИО, статус, цветовая маркировка каждого гостя. Данные из GET /v1/search/guests_in_casino | - |
| Кнопка выбора | Кнопка (по строке) | Выбор гостя из списка для идентификации | - |
...
Полное описание каждого метода (параметры, тело запроса, заголовки, JSON-примеры, ошибки): [MGS-API-справочник.md](MGS-API-справочник.md)
...
| image | string | Нет | URL фотографии гостя (HTTPS). Отсутствует в ответе guests_in_casino | Валидный URL |
...
| Поле | Тип | Обязательное | Описание | Ограничения |
|------|-----|:------------:|----------|-------------|
| point_id | integer (int32) | Да | ID типа баллов. В плагине используются два типа: Restaurant (из параметра конфигурации RestaurantPointId) и Complimentary (из ComplimentaryPointId). Значения из openapi.yaml: 0=Complimentary, 1=Игра, 3=Балл Путешественника, 4=Ресторан | Значения параметризованы в конфиге |
...
| Поле | Тип | Обязательное | Описание | Ограничения |
|------|-----|:------------:|----------|-------------|
| point_id | integer (int32) | Да | ID типа баллов. Значение берётся из параметра конфигурации: RestaurantPointId (для ресторанных баллов) или ComplimentaryPointId (для комплиментарных). Значения из openapi.yaml: 0=Complimentary, 1=Игра, 3=Балл Путешественника, 4=Ресторан | Должен соответствовать типу из BalanceResponse |
...
> [!NOTE]
> Полный маппинг по каждому методу API с указанием источников данных для каждого поля см. в [MGS-API-справочник.md](MGS-API-справочник.md), раздел 6.
...
> [!WARNING]
> Скидки отложены за пределы MVP (решение встречи 11.03.2026). Параметры сохранены в конфигурации для будущей реализации, но не обрабатываются плагином в MVP.
...
| use_comp_role | string | Нет | (пусто = все) | Роли, которым разрешена оплата комплиментарными баллами | 8.10, п.5 |
...
| show_comp_role | string | Нет | (пусто = все) | Роли, которым виден баланс комплиментарных баллов | 8.10, п.10 |
...
| comp_button | string | Нет | Списать комплиментарные баллы | Текст кнопки оплаты комплиментарными баллами | 8.11, п.3 |
...
| comp_button_background | string | Нет | #ffffff | Цвет фона кнопки комплиментарных (HEX) | 8.11, п.4 |
| comp_button_foreground | string | Нет | #000000 | Цвет текста кнопки комплиментарных (HEX) | 8.11, п.4 |
...
| RestaurantPointId | int | Нет | 4 | ID типа баллов "Ресторан" в MGS. Значение по умолчанию из openapi.yaml (point_id=4). НЕ хардкодить - на разных объектах казино значения могут отличаться | 8.6, п.1 |
| ComplimentaryPointId | int | Нет | 0 | ID типа баллов "Complimentary" в MGS. Значение по умолчанию из openapi.yaml (point_id=0). НЕ хардкодить | 8.6, п.4 |
...
<configuration>
  ...
  <applicationSettings>
    <Resto.Front.Api.NeonPlugin.Properties.Settings>
      ...
      <setting name="use_comp_role" serializeAs="String">
        <value></value>
      </setting>
      ...
      <setting name="show_comp_role" serializeAs="String">
        <value></value>
      </setting>
      ...
      <setting name="comp_button" serializeAs="String">
        <value>Списать комплиментарные баллы</value>
      </setting>
      ...
      <setting name="comp_button_background" serializeAs="String">
        <value>#ffffff</value>
      </setting>
      <setting name="comp_button_foreground" serializeAs="String">
        <value>#000000</value>
      </setting>
      ...
      <setting name="RestaurantPointId" serializeAs="String">
        <value>4</value>
      </setting>
      <setting name="ComplimentaryPointId" serializeAs="String">
        <value>0</value>
      </setting>
...
> [!NOTE]
> Форматы ошибок API MGS стабильны и не изменятся в текущей версии (решение встречи 11.03.2026). Плагин МОЖЕТ полагаться на задокументированные HTTP-коды ответов.
...
- Замена Neon на MGS поэтапная, первый объект - март 2026. На переходном этапе разные объекты заказчика могут работать с разными системами (Neon или MGS). *(Решение по итогам встречи с заказчиком)*
- Доступ к тестовой среде MGS только через L2TP VPN. *(Решение по итогам встречи с заказчиком)*
...
- Endpoint возврата средств (refund) отсутствует в текущей версии API MGS. Возврат кэш-поинтов и баллов лояльности реализуется через систему заявок в кассу казино, а не через API плагина (решение встречи 11.03.2026). Функция возврата отложена за пределы MVP. *(API MGS, решение встречи 11.03.2026)*
...
- [ДОПУЩЕНИЕ] Пользовательские сценарии (UI, действия кассира) не меняются. Если окажется иначе - потребуется переработка разделов 7 и 9. *(Решение по итогам встречи с заказчиком)*
- [ДОПУЩЕНИЕ] Ролевая модель текущего плагина сохраняется без изменений. *(Следствие из сохранения пользовательских сценариев)*
- [ДОПУЩЕНИЕ] Логика скидок по статусу гостя отложена за пределы MVP (решение встречи 11.03.2026: "сейчас у нас нету скидок"). Параметры конфигурации сохранены для будущей реализации. *(Решение встречи 11.03.2026)*
...
- API MGS поддерживает только списание баллов (DEBIT). Начисление баллов (CREDIT) через API невозможно - начисление выполняется только со стороны РКЦ (расчётно-кассовый центр казино). *(Решение встречи 11.03.2026)*
- Форматы ошибок API MGS стабильны и не изменятся в текущей версии. Плагин МОЖЕТ полагаться на задокументированные HTTP-коды ответов. *(Решение встречи 11.03.2026)*
- Значения point_id (RestaurantPointId, ComplimentaryPointId) могут отличаться на разных объектах казино. Значения НЕ ДОЛЖНЫ быть захардкожены - настраиваются через конфигурационный файл. *(Решение встречи 11.03.2026)*
