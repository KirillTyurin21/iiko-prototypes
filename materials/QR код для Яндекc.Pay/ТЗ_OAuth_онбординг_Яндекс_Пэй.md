...
> **Jira:** OUT-1605 (задача) / OUT-1403 (эпик)
> **Confluence:** https://wiki.iiko.ru/pages/viewpage.action?pageId=147245972
...
|---|----------|-------------|
| 1 | Анализ доработки OAuth-онбординг Яндекс.Пэй | /02_Спецификации/Плагин_iikoFront/Анализ_доработки_OAuth_онбординг_Яндекс_Пэй.md |
| 2 | Постановка задачи (спецификация от 19.03.2026) | /01_Входящие_документы/Постановка задачи/Задача от 19.03.2026/ |
| 3 | OpenAPI-спецификация Vendor Cabinet API | schema_upd.html (получена от Яндекс 18.03.2026) |
| 4 | Техническое задание QR-код от Яндекс.Пэй | /02_Спецификации/Плагин_iikoFront/Техническое задание QR-код от Яндекс.Пэй.md |
| 5 | ТЗ по областям (текущий iikoWeb) | /05_Рабочие_материалы/Работа с QR табличками/ФАЙЛЫ QR/ТЗ по областям.md |
| 6 | Инструкция Яндекс.Пей | /01_Входящие_документы/Инструкции из iikoHelp/Инструкция Яндекс.Пей.md |
| 7 | API.md (платежное API Яндекс.Пэй) | /05_Рабочие_материалы/API.md |
| 8 | Документация Яндекс ID (OAuth 2.0) | https://yandex.ru/dev/id/doc/ru/ |
| 9 | Confluence: Архитектура системы Яндекс.Пэй | https://wiki.iiko.ru/pages/viewpage.action?pageId=147244596 |
| 10 | Справочник iiko и источники данных | /01_Входящие_документы/Документация_iiko/05_справочник_iiko_и_источники_данных.md |

**Трекинг задачи:** Jira OUT-1605 (эпик OUT-1403), Confluence: https://wiki.iiko.ru/pages/viewpage.action?pageId=147245972
...
- Массовая подача заявок на подключение (до 20 ресторанов за один запрос)
...
4. **Подача заявки на подключение ресторанов** (Merchant) - до 20 в одной заявке, с указанием MCC-кода
...
### 2.6. Участники проекта

| Роль | Имя | Ответственность |
|------|-----|----------------|
| Системный аналитик | Кирилл | Спецификация, анализ, координация |
| Руководитель | Руслан | Приемка, приоритизация |
| Техлид | Наиль | Архитектурные решения, реализация |
| Яндекс.Пэй | Даниил Логатский | Контактное лицо, API, согласования |
| Яндекс.Пэй | Гоша | Техническая поддержка API |

### 2.7. Ключевые даты

| Дата | Событие |
|------|------|
| 18.02.2026 | 1-я встреча с Яндексом (архитектура OAuth) |
| 18.03.2026 | 2-я встреча, получена актуальная YAML-схема Vendor Cabinet API |
| 19.03.2026 | Начало подготовки спецификации |
| 20.03.2026, 13:00 | Встреча с Яндексом (уточнение открытых вопросов) |
| 23.03-06.04.2026 | Отпуск Кирилла (SA) |
| 01.04.2026 | Ожидаемая готовность API со стороны Яндекс |
...
<!-- Секция не заполнена в текущей версии. Функциональные требования распределены по соответствующим секциям:
     - UI-спецификация: секции 8 (Пользовательские сценарии) и 9 (Интерфейс пользователя)
     - Backend: секция 10 (Доработка iikoWeb Backend)
     - Плагин iikoFront: секция 11 (не заполнена, см. Шаг 8 плана реализации)
     Сводный перечень FR-xxx планируется в следующей версии. -->

---

## 5. Нефункциональные требования

<!-- Секция не заполнена в текущей версии. Отдельные нефункциональные требования описаны в:
     - Безопасность: секция 6.6 (требования к безопасности OAuth), секция 10.5 (безопасность Backend)
     - Производительность: секция 10.6 (кеширование)
     - Тестирование: секция 15 (тестовые сценарии и критерии приемки)
     Сводный перечень NFR-xxx планируется в следующей версии. -->
...
| 3 | merchants | POST | /merchants/register | Создать заявки на подключение мерчантов (до 20 за запрос) |
...
Детальное описание каждого метода (параметры, JSON-схемы запросов/ответов, curl-примеры, коды ошибок) вынесено в отдельный документ: [Vendor_Cabinet_API_методы.md](Vendor_Cabinet_API_методы.md)
...
| 9 | Выбирает рестораны из дерева iiko для подключения | Открывается форма заявки: выбранные рестораны, MCC-код (dropdown), банковские реквизиты, контакт | - |
| 10 | Заполняет данные заявки, нажимает "Подать заявку" | Toast: "Заявка подана". Статус мерчантов = "processing" | `POST /merchants/register` (partner_id, массив merchants до 20 шт.) |
...
| 12 | - (автоматически, после одобрения) | Поллинг возвращает registration_status: "active". Статус мерчанта меняется на "Подключен" (зеленый). Появляется секция маппинга QR-табличек | `GET /merchants/{id}/status` |
...
|---|------|-----|-------------|-----------|----------------------------|
| 1 | Название организации | text input | Да | Не пустое | `name` |
| 2 | ИНН | text input | Да | 10 или 12 цифр | `registration_data.tax_ref_number` |
| 3 | ОГРН | text input | Да | 13 или 15 цифр | `registration_data.ogrn` |
| 4 | КПП | text input | Нет | 9 цифр | `registration_data.kpp` |
| 5 | Юридический адрес | text input | Да | Не пустое | `registration_data.legal_address` |
...
| 11 | Контакт: Email | text input | Да | Формат email | `contact.email` |
| 12 | Контакт: Телефон | text input | Да | Формат телефона | `contact.phone` |
| 13 | Контакт: Фамилия | text input | Да | Не пустое | `contact.last_name` |
| 14 | Контакт: Имя | text input | Да | Не пустое | `contact.first_name` |
...
| Параметр | Описание |
|----------|----------|
| Источник списка | Список касс загружается из iiko (RMS) |
| Формат выбора | Чекбоксы или мультиселект - пользователь отмечает кассы, на которые нужно подключить QR-код |
...
| Кнопка | Действие |
|--------|----------|
| "Подать заявку" | Отправляет `POST /merchants/register`. При нажатии - spinner, блокировка полей |
...
- При переходе в статус `active` - поллинг прекращается, автоматически запрашивается User Token через `POST /merchants/{id}/usertoken`, полученный токен сохраняется в iiko.
- При переходе в статус `failed` - поллинг прекращается, отображается предупреждение.
- Если пользователь закрывает страницу во время начального поллинга - при следующем открытии статус запрашивается повторно (стратегия 2).
...
| Привязка табличек к кассам | Мгновенное сохранение, dropdown, toast | Без изменений |
...
<!-- Секция не заполнена. Соответствует Шагу 8 плана реализации (Анализ_доработки, секция 13).
     Источники для заполнения:
     - ТЗ QR-код от Яндекс.Пэй (существующий плагин)
     - Инструкция Яндекс.Пей
     - API.md (платежное API)
     - Анализ лога плагина
     Содержание: автоматическое получение User Token через backend, обратная совместимость, fallback, изменения конфига. -->

---

## 12. Обработка ошибок и граничные случаи

<!-- Секция не заполнена в текущей версии. Обработка ошибок описана в:
     - OAuth-ошибки: секция 6.7 (обработка ошибок OAuth)
     - API-ошибки: Vendor_Cabinet_API_методы.md, секция 7.7 (коды ошибок API)
     - Sequence-диаграмма ошибок: секция 16.2
     - Негативные тест-кейсы: секция 15.3 (TC-101..TC-115)
     - Граничные случаи: секция 15.4 (TC-201..TC-210)
     Сводная таблица обработки ошибок планируется в следующей версии. -->
...
| TC-003 | Создание организации (нового партнера) | 1. Пользователь авторизован через OAuth. 2. Организация с данным ИНН не существует в Яндекс.Пэй | 1. Нажать "Создать организацию". 2. Заполнить обязательные поля: название организации, ИНН (10 или 12 цифр), ОГРН (13 или 15 цифр), юридический адрес, email, телефон, фамилия, имя. 3. Опционально заполнить: КПП, почтовый адрес, почтовый индекс, полное наименование, ФИО руководителя, URL сайта, отчество. 4. Нажать "Создать" | POST /partners выполнен успешно. Организация появляется в списке. partner_id сохранен. Форма закрывается, отображается список мерчантов (пустой) | 8.1 (шаги 5-7) |
...
| TC-006 | Батч-подача заявок (несколько ресторанов) | 1. Организация создана. 2. В iiko зарегистрировано несколько торговых точек (минимум 3) | 1. Нажать "Подать заявку на подключение". 2. Выбрать несколько ресторанов (например 3). 3. Заполнить данные для каждого ресторана (MCC-код, банковские реквизиты, контакт). 4. Нажать "Подать заявку" | POST /merchants/register выполнен с массивом merchants[] (до 20 элементов). Все мерчанты появляются в списке со статусом "На рассмотрении". Для каждого сохранен merchant_id. Поллинг запущен для каждого мерчанта | 8.1 (батч-режим) |
...
| TC-203 | Максимум 20 мерчантов в одном батче | Организация создана, в iiko ровно 20 ресторанов не привязаны | 1. Выбрать 20 ресторанов для регистрации. 2. Нажать "Подать заявку" | POST /merchants/register отправлен с массивом из 20 элементов. Запрос принят (200 OK). Все 20 мерчантов появляются со статусом "На рассмотрении" | Граничный: лимит батча = 20 |
| TC-204 | Превышение лимита батча (> 20 мерчантов) | Организация создана, в iiko более 20 ресторанов не привязаны | 1. Попытаться выбрать 21+ ресторан для регистрации | UI не позволяет отправить более 20 за один запрос. Сообщение: "Максимум 20 ресторанов за одну заявку". Кнопка "Подать заявку" неактивна при превышении лимита | Граничный: превышение лимита |
...
| 8.1 | AC-107 | Форма создания организации содержит все обязательные поля: название, ИНН, ОГРН, юр. адрес, email, телефон, имя, фамилия | P0 |
| 8.1 | AC-108 | POST /partners с заполненными данными создает организацию; при успехе организация появляется в списке | P0 |
| 8.1 | AC-109 | POST /merchants/register с выбранными ресторанами (до 20 шт.) создает заявку на подключение мерчантов | P0 |
...
| AC-G09 | Батч-ограничение мерчантов | P1 | POST /merchants/register принимает массив до 20 мерчантов за один запрос |
...
| 3 | Конкретные scope OAuth-приложения для Vendor Cabinet API (в документации не раскрыты) | Яндекс (Гоша / Даниил Логатский) | 20.03.2026 | Открыт |
| 4 | Формат тела ошибки HTTP 400 (по полям или общий текст?) | Яндекс (Гоша) | 20.03.2026 | Открыт |
...
|--------|------|-------|--------------------|
| 0.1 | 19.03.2026 | Кирилл | Каркас документа: структура 18 секций, содержание, заглушки |
| 0.2 | 19.03.2026 | Кирилл | Шаг 1: Введение (секция 1), Общее описание (секция 2), Глоссарий (секция 3), Ограничения (секция 13), Открытые вопросы (секция 17) |
| 0.3 | 19.03.2026 | Кирилл | Шаг 2: Авторизация OAuth 2.0 (секция 6) - подсекции 6.1-6.9. Authorization Code Flow, регистрация приложения, управление токенами, кнопка "Войти с Яндекс ID", безопасность, обработка ошибок, ограничения, PlantUML sequence-диаграмма |
| 0.4 | 19.03.2026 | Кирилл | Шаг 3: Vendor Cabinet API (секция 7) - подсекции 7.1-7.7. Общие сведения, сводная таблица 9 методов, группы Partners (2 метода), Merchants (3 метода), UserToken (3 метода), Reference (1 метод), коды ошибок API. Для каждого метода: параметры, JSON-схемы запросов/ответов, curl-примеры, особенности. Верификация - 0 ошибок, 54/54 проверки PASS |
| 0.4.1 | 19.03.2026 | Кирилл | Рефакторинг секции 7: детальные описания методов (7.3-7.7) вынесены в отдельный файл Vendor_Cabinet_API_методы.md. В основном ТЗ оставлены общие сведения (7.1) и сводная таблица (7.2) со ссылкой на файл методов |
| 0.5 | 19.03.2026 | Кирилл | Шаг 4: UI-спецификация экранов. Заполнены секции 8 "Пользовательские сценарии" (подсекции 8.1-8.8: основной сценарий, идемпотентность ИНН, повторная авторизация, чужой аккаунт, отклонение, перепривязка QR, новые кассы, смена аккаунта) и 9 "Интерфейс пользователя" (подсекции 9.1-9.8: текущий экран, блок авторизации, список организаций, форма организации 15 полей, форма мерчанта 11 полей, экран статусов с 2 стратегиями поллинга, маппинг QR-табличек, кнопка Яндекс ID SDK). Верификация 1 (19.03): 3 MED + 5 LOW (секция 8) и 2 MED + 5 LOW (секция 9) исправлены. Верификация 2 (20.03, дополнительная): 6 LOW (секция 8) исправлены; секция 9 - 0 ошибок. Повторная проверка 6/6 - PASS |
| 0.6 | 20.03.2026 | Кирилл | Шаги 5-6: Sequence-диаграммы (секция 16). Подсекция 16.1 "Основной сценарий онбординга (Happy Path)" - PlantUML диаграмма 17 шагов: OAuth -> Partner -> Merchant -> поллинг -> User Token -> QR-маппинг. 5 участников, 13 API-вызовов. Верификация: 23/28 PASS, 2 MED (обертки ответов partners/merchants) + 3 LOW (имена полей) исправлены. Подсекция 16.2 "Обработка ошибок при онбординге" - PlantUML диаграмма с alt/opt блоками: 5 ошибок OAuth, 5 ошибок API (400-500) с refresh_token, отклонение мерчанта. Верификация: 33/35 PASS, 1 MED (слово "автоматического") + 1 LOW (описание из ответа API) исправлены. Дополнительная верификация 20.03.2026: Шаг 5 - 55 проверок, 2 MED (вложенность POST /merchants/register + poses_count: 0) исправлены, 4/4 re-check PASS; Шаг 6 - 54 проверки, 1 MED ("Прекратить поллинг") исправлен, 5/5 re-check PASS |
| 0.7 | 20.03.2026 | Кирилл | Шаг 9: Тестирование (секция 15). Подсекции 15.1-15.5: тестовое окружение,  16 позитивных сценариев (TC-001..TC-016), 15 негативных (TC-101..TC-115), 10 граничных (TC-201..TC-210), 60 критериев приемки (46 по сценариям + 14 общих). Верификация: 12 ошибок (1 HIGH, 8 MED, 3 LOW), 9 исправлены, 3 LOW допустимы |
| 0.8 | 20.03.2026 | Кирилл | Шаг 10: Обратная совместимость и миграция (секция 14). Подсекции 14.1-14.3: 14.1 стратегия обратной совместимости (5 принципов, 4 гарантии), 14.2 матрица совместимости (9 аспектов), 14.3 совместимость по компонентам (7 компонентов). Верификация: 5 ошибок (1 HIGH, 1 MED, 3 LOW), 3 исправлены, 2 LOW вне скоупа |
| 0.8.1 | 20.03.2026 | Кирилл | Исправлена ссылка на Postman-коллекцию в секции 15.1: "Коллекция из секции 14 данного ТЗ" заменена на "Коллекция из секции 14 документа Анализ доработки OAuth-онбординг" |
| 0.8.2 | 20.03.2026 | Кирилл | Полный аудит документа. Исправлено 7 проблем: (1) версия в заголовке 0.6 -> 0.8.1; (2-5) заглушки секций 4, 5, 11, 12 заменены на перекрестные ссылки к заполненным секциям и пометки о незаполненности; (6) в секции 6.2 уточнен redirect_uri = `https://{iiko-web-domain}/api/oauth/callback`; (7) в секции 9.6 добавлено предупреждение о расхождении Стратегии 1 поллинга (3-10 сек) с рекомендацией Vendor Cabinet API (10 мин) - требует согласования с Яндекс. Статус шагов плана: 1-7 и 9-10 ВЫПОЛНЕНЫ, Шаг 8 (плагин iikoFront, секция 11) НЕ ВЫПОЛНЕН |
