...
Независимо от используемой модели, следуй этим дополнительным правилам:

- **Перед генерацией кода** — прочитай аналогичный существующий файл в проекте и следуй его паттерну
- **После генерации кода** — проверь каждый import: он должен быть из `@/components/ui`, `@/shared/...` или `@angular/...`
- **Перед завершением задачи** — пройди по соответствующему чеклисту (см. секцию «Чеклисты»)
- **При любом сомнении** — предпочитай копирование из существующего кода вместо генерации с нуля
...
Каждый прототип имеет структурированный TypeScript-файл `changelog.data.ts` в своей папке. Changelog **отображается в UI** прототипа — в top-bar показана версия (без даты), по клику открывается модальное окно с полной историей. **Дата НЕ отображается в UI** — ни в top-bar, ни в модалке. Поле `date` в данных сохраняется для внутреннего учёта.
...
> Полные шаблоны вынесены в **`files/TEMPLATES.md`** — открой файл и копируй шаблоны с заменой плейсхолдеров `<slug>`, `<SLUG_UPPER>`, `<Name>`, `<description>`.
>
> Содержит шаблоны: `routes.ts`, `prototype.component.ts`, `changelog.data.ts`, `main-screen.component.ts`, а также фрагменты для `app.routes.ts`, `prototypes.registry.ts`, `changelog-button.component.ts`.
...
>
> Полный справочник вынесен в **`files/UI_API_REFERENCE.md`** — открой файл перед использованием UI-компонентов.
>
> Компоненты: UiButton, UiCard, UiCardHeader, UiCardTitle, UiCardContent, UiInput, UiSelect, UiTextarea, UiCheckbox, UiToggle, UiModal, UiConfirmDialog, UiTable, UiTabs, UiBadge, UiAlert, UiBreadcrumbs, UiStatusDot, UiEmptyState, UiDivider, UiSkeleton.
